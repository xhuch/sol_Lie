...
def matrix_group_signature(ca, m: int) -> Dict[str, object]:
    center_idx = standard_center_indices(ca, m)
    v_idx = [i for i in range(m) if i not in set(center_idx)]
...
def bracket_tensor_profile(ca, m: int) -> Dict[str, object]:
...
def extension_action_profile(ca, m: int, n: int, level: str = "quick") -> Dict[str, object]:
    r = n - m
    actions = []
    ...
    joint_plane = None
    if len(actions) == 2:
        A1 = sp.Matrix([[ca[j][m][i] for j in range(m)] for i in range(m)])
        A2 = sp.Matrix([[ca[j][m + 1][i] for j in range(m)] for i in range(m)])
...
def run_pair_pipeline(args_dict: Dict[str, object]) -> Dict[str, object]:
    ...
    bt1 = bracket_tensor_profile(rep1.adapted_structure_constants, rep1.n_basis_dim)
    bt2 = bracket_tensor_profile(rep2.adapted_structure_constants, rep2.n_basis_dim)
    mg1 = matrix_group_signature(rep1.adapted_structure_constants, rep1.n_basis_dim)
    mg2 = matrix_group_signature(rep2.adapted_structure_constants, rep2.n_basis_dim)

    ext1 = extension_action_profile(
        rep1.adapted_structure_constants,
        rep1.n_basis_dim,
        rep1.n,
        level=str(args_dict.get("extension_profile_level", "quick")),
    )
...
def run_batch_pipeline(args_dict: Dict[str, object]) -> Dict[str, object]:
    ...
    for path in files:
        ...
        bt = bracket_tensor_profile(rep.adapted_structure_constants, rep.n_basis_dim)
        mg = matrix_group_signature(rep.adapted_structure_constants, rep.n_basis_dim)
